...
/**
 * Опорное «сейчас» для демо-обсуждений (стабильные «N ч назад» при SSR).
 * Совпадает с контекстом сидов (апрель 2026).
 */
...
/**
 * Временные данные, имитирующие ответ БД для текущего пользователя.
 * Заменить на реальные запросы при подключении API.
 */
...
export type ClientRecord = {
  ...
  /** ФИО */
  fullName: string;
  /** Ник в Instagram (без @ в данных; при показе можно добавить @) */
  instagramUsername: string;
  /** Всего опубликованных / запланированных постов в системе */
  postsTotal: number;
  /** Постов с датой в текущем календарном месяце */
  postsThisMonth: number;
  /** На модерации / согласовании */
  postsPendingReview: number;
  /** Сфера деятельности, 1–2 пункта */
...
export type PostDraftRecord = {
  id: string;
  clientId: string;
  /** Статус в согласовании и публикации */
  ...
  firstComment: string;
  altText: string;
  /** URL медиа (сид: внешние заглушки; в проде — из хранилища) */
  imageUrls: string[];
  /** `input[type=date]` (локаль) */
  publishDate: string;
  /** `input[type=time]` (локаль) */
  publishTime: string;
  /** Когда запись «создана» в системе */
  createdAt: string;
  /** Обсуждение согласования с клиентом (хронологический порядок). */
  discussion?: PostReviewComment[];
  /** Секретная часть URL клиентского просмотра (`/review/...`). */
  clientReviewToken: string;
};

/**
 * Посты, как будто сохранены с экрана создания.
 * @see `/posts/new`
 */
export const mockPostDrafts: PostDraftRecord[] = [
  {
    id: "p1",
    clientId: "c1",
    clientReviewToken:
...
/** Московская отметка времени публикации для демо-сравнений (согласовано с MOCK_DISCUSSION_REF_MS). */
...
export function countScheduledPostsInNextDays(
  days: number,
  refMs: number = MOCK_DISCUSSION_REF_MS
): number {
  const end = refMs + days * DAY_MS;
  return mockPostDrafts.filter(
    (p) =>
      p.status === "scheduled" &&
...
/** Запланированные посты от «сейчас» демо, ближайшие первые. */
export function getUpcomingScheduledPosts(
  limit: number,
  refMs: number = MOCK_DISCUSSION_REF_MS
): PostDraftRecord[] {
  return [...mockPostDrafts]
    .filter((p) => p.status === "scheduled" && scheduledPublishMs(p) >= refMs)
    .sort((a, b) => scheduledPublishMs(a) - scheduledPublishMs(b))
    .slice(0, limit);
}

/** Значения формы редактора (без id, статуса и служебных полей записи). */
export type PostEditorInitialValues = Omit<
  PostDraftRecord,
  "id" | "createdAt" | "status" | "discussion" | "clientReviewToken"
>;

export function getMockPostDraftById(id: string): PostDraftRecord | undefined {
  return mockPostDrafts.find((p) => p.id === id);
}

export function getMockPostDraftByClientReviewToken(
  token: string
): PostDraftRecord | undefined {
  const t = decodeURIComponent(token).trim();
  if (!t) return undefined;
  return mockPostDrafts.find((p) => p.clientReviewToken === t);
}
...
/** Тип строки ленты активности (дашборд). */
...
/** Событие из «БД» для блока недавней активности. */
export type RecentActivityRecord = {
  id: string;
  kind: ActivityKind;
  /** Момент события (epoch ms), в шкале демо. */
  createdAtMs: number;
  title: string;
  detail?: string;
  clientId?: string;
...
/**
 * Лента активности (имитация таблицы событий).
 * `createdAtMs` ≤ MOCK_DISCUSSION_REF_MS — всё уже «произошло» к демо-сейчас.
 */
export const mockRecentActivities: RecentActivityRecord[] = [
  {
    id: "act-01",
    kind: "client_comment",
...
/**
 * Показатели дашборда: агрегаты по клиентам и черновикам + константы-«строки БД», где полного списка постов нет.
 */
export function getDashboardStats(
  refMs: number = MOCK_DISCUSSION_REF_MS
): DashboardStat[] {
  const totalPosts = mockClients.reduce((s, c) => s + c.postsTotal, 0);
  const weekMs = 7 * 24 * 60 * 60 * 1000;
  const weekStart = refMs - weekMs;
  const postsCreatedThisWeek = mockPostDrafts.filter((p) => {
    const t = Date.parse(p.createdAt);
    return !Number.isNaN(t) && t >= weekStart && t <= refMs;
  }).length;
  const inReview = mockClients.reduce((s, c) => s + c.postsPendingReview, 0);
  const activeClients = mockClients.length;
  const scheduledNext30 = countScheduledPostsInNextDays(30, refMs);
