...
export function VkIdTokenButton({ onAccessToken, disabled }: VkIdTokenButtonProps) {
  ...
  useEffect(() => {
    ...
    if (!appIdRaw || !redirectUrl) {
      setHint(
        "Задайте в .env переменные NEXT_PUBLIC_VK_APP_ID и NEXT_PUBLIC_VK_REDIRECT_URL (тот же базовый URL, что указан в настройках приложения VK → доверенные redirect URI).",
      );
      return;
    }
  ...
  return (
    <div className="space-y-2">
      <p className="text-[13px] font-medium text-[var(--foreground)]">
        Войти через VK ID и подставить токен
      </p>
      {hint ? (
        <p className="text-[12px] leading-relaxed text-[var(--muted)]">{hint}</p>
      ) : (
        <div
          ref={containerRef}
          className="min-h-[52px] w-full rounded-xl border border-[var(--border)] bg-[var(--background)] px-2 py-2"
        />
      )}
      {localError ? (
        <p
      ...
      {tokenOk ? (
        <p className="text-[12px] text-emerald-200/90">
          Токен получен и подставлен в поле ниже — при необходимости сохраните клиента.
        </p>
      ) : null}
    </div>
  );
