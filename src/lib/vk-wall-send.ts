...
function formatVkApiError(err: VkApiError): string {
  ...
  if (code === 15) {
    return (
      "У токена нет прав на этот метод API (ош. 15). Для стены и фото нужны права «wall» и «photos» (иногда «groups»). " +
      "Токен из кнопки VK ID часто выдаётся только с профилем: в разделе «Доступы» id.vk.ru обычно нет отдельных галочек wall/photos — это нормально. " +
      "Попробуйте NEXT_PUBLIC_VK_SCOPE с пробелами (например vkid.personal_info wall photos groups offline) и повторный вход с аккаунта ВКонтакте; " +
      "если не помогло — ключ доступа группы (для своей стены группы), пользовательский токен с dev.vk.com вручную или devsupport@corp.vk.com (см. документацию VK ID по расширенным доступам)."
    );
  }
  if (code === 1051) {
    return (
      "Метод недоступен для этого типа профиля (ош. 1051). Чаще всего: входили через VK ID не аккаунтом ВКонтакте, а через Mail или Одноклассники — такой токен не подходит для wall.post на стене ВК. " +
      "Выйдите и войдите снова, выбрав именно ВКонтакте. Для поста в группу можно использовать ключ доступа сообщества (Настройки группы → Работа с API) вместо пользовательского токена. " +
      "Если вход только с ВК, а ошибка остаётся — проверьте тип приложения и доступы в кабинете VK ID или обратитесь в поддержку разработчиков."
    );
  }
